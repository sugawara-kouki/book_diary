  ...
  plan: [
    {
      title: "雪国",
      author: "川端康成",
      coverUrl: "https://images.unsplash.com/photo-1476275466078-4007374efbbe?q=80&w=800",
      progress: 0
    },
    {
      title: "銀河鉄道の夜",
      author: "宮沢賢治",
      coverUrl: "https://images.unsplash.com/photo-1472214103451-9374bd1c798e?q=80&w=800",
      progress: 0
    }
  ]
}
    ...
    <Shell>
      <div className="space-y-8">
        ...
        <Tabs defaultValue="all" className="space-y-6">
          ...
          <TabsContent value="all" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {allBooks.map((book, index) => (
              <BookCard key={index} {...book} />
            ))}
          </TabsContent>

          <TabsContent value="reading" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {books.reading.map((book, index) => (
              <BookCard key={index} {...book} />
            ))}
          </TabsContent>

          <TabsContent value="completed" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {books.completed.map((book, index) => (
              <BookCard key={index} {...book} />
            ))}
          </TabsContent>

          <TabsContent value="plan" className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
